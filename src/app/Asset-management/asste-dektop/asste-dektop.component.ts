...
@Component({
  selector: 'app-asste-dektop',
  templateUrl: './asste-dektop.component.html',
  styleUrls: ['./asste-dektop.component.css'],
 ...
 type= '';
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      astg_group: ['DESKTOP', Validators.required],
...
servicp(os){
  ...
  onSubmit() {
   ...
   this.form.value.fastno = this.form.value.fastno.trim();
    ...
    // this.empid1 = this.form.value.name.username.split('/');
    // console.log(this.empid1)
    this.form.value.name = this.empid1[0];
    this.form.value.empid = this.empid1[1];
    console.log(this.form.value.name,this.empid1[1],this.empid1[0],this.empid1)
  ...
  assetDesktop(){
    ...
    this.assetService.assetDesktopInsert(formData)
      .subscribe(res => {
      //  this.form.value.name.username = this.form.value.name+'/'+ this.form.value.empid;
        ...
        if(res === 'Asset Created Succesfully')
        {
          ...
          this.alterservices.success('Asset Created successfully', {  autoClose: true, keepAfterRouteChange: true });       
